...
void ParticleSystem::update(bool spawnParticles, const Vector3f& spawnPosition, float dt, const Vector3f& magneticField_1, const Vector3f& magneticField_2, const Vector3f& magneticField_3, const Vector3f& magneticField_4) {
    ...
    //--------------------------------------------------------------------------
    for ( std::size_t i = 0; i < this->particles.size(); i++ ) {
        this->particles[i].velocity += (dt * this->particles[i].force * 1.0f / this->particles[i].mass);
        ...
        if ( spawnParticles && lifetime < 0.0f ) {
			...
			this->particles[i].velocity = Vector3f(0.0f, 0.0f, -30.0f);
        ...
        //--------------------------------------------------------------------------
			...
			if (particles[i].position.getZ() < 10.0f) {
				this->particles[i].force = magneticField_1;
			}
			if (particles[i].position.getZ() < 1.0f) {
				this->particles[i].force = magneticField_2;
			}
			if (particles[i].position.getZ() < 0.0f) {
				this->particles[i].force = magneticField_3;
			}
			if (particles[i].position.getZ() < -2.5f) {
				this->particles[i].force = magneticField_4;
			}
			if (particles[i].position.getZ() < -15.0f) {
				this->particles[i].force = Vector3f::Zero();
			}
        }
...
void ParticleSystem::setBounceEnergy(float energy) {
	this->bounceEnergy = energy;
}
void ParticleSystem::setInitVelocity(float velocity) {
	this->initVelocity = velocity;
}
void ParticleSystem::setMinLifetime(float min) {
	this->minLifetime = min;
}
void ParticleSystem::setMaxLifetime(float max) {
	this->maxLifetime = max;
}
void ParticleSystem::setColor(float r, float g, float b) {
	color.setR(r);
	color.setG(g);
	color.setB(b);
}
